...
export async function action({ request }: { request: Request }) {
  ...
  try {
    schedulingService.partySubmitDecisions(DEFAULT_CASE_ID, "BEKLAGTER", decisionMap);
    return null;
  } catch (err) {
...
export function meta() {
  return [{ title: "Beklagter - Court Appointment Scheduling" }];
}

export default function BeklagterRoute() {
  const { user, overview } = useLoaderData<typeof loader>();
  return <PartyScreen role="BEKLAGTER" overview={overview} user={user} />;
}

export function ErrorBoundary() {
  return <RouteErrorBoundary title="Beklagter - Response" />;
}
